...
export default function TestSeries({courseId,onStart}){
 ...
 async function create(){
  setMessage('');const parsed=mode==='paste'?parseQuestions(raw):[],parsedHi=mode==='paste'?parseQuestions(rawHi):[];if(!title.trim())return setMessage('Add a test title first.');if(mode==='paste'&&!parsed.length)return setMessage('No complete MCQs detected. Use Q1 + A/B/C/D + Answer: B.');
