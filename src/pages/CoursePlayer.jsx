...
export default function CoursePlayer({courseId,videoId,back}){
 ...
 useEffect(()=>{if(!v)return;let mounted=true;const id=youtubeId(v.youtube_video_id||v.video_url||v.url);if(!id)return;ensureYouTubeApi().then(YT=>{if(!mounted||!playerHost.current)return;player.current=new YT.Player(playerHost.current,{videoId:id,width:'100%',height:'100%',playerVars:{rel:0,modestbranding:1,playsinline:1},events:{onReady:e=>{if(progress[v.id]?.last_position_seconds)e.target.seekTo(progress[v.id].last_position_seconds,true)},onStateChange:e=>{const Y=window.YT;if(e.data===Y.PlayerState.PLAYING){setPlaying(true);last.current=e.target.getCurrentTime();lastWall.current=Date.now();clearInterval(timer.current);timer.current=setInterval(()=>{const now=e.target.getCurrentTime(),delta=Math.max(0,now-(last.current??now)),wall=Math.max(0,(Date.now()-(lastWall.current??Date.now()))/1000),rate=typeof e.target.getPlaybackRate==='function'?Number(e.target.getPlaybackRate())||1:1,expected=wall*rate;const valid=delta<=Math.max(8,expected*2+1);if(delta>0&&valid){const before=watchedRef.current;watchedRef.current+=delta;setWatched(watchedRef.current);const beforeXp=Math.round(before/60*XP_PER_MINUTE),afterXp=Math.round(watchedRef.current/60*XP_PER_MINUTE);if(afterXp!==beforeXp)setGlobalXp(x=>x+(afterXp-beforeXp));save(now,e.target.getDuration())}last.current=now;lastWall.current=Date.now()},2000)}else{setPlaying(false);clearInterval(timer.current);if(e.data===Y.PlayerState.ENDED)finish(e.target.getDuration())}}}});}).catch(()=>{});return()=>{mounted=false;clearInterval(timer.current);try{player.current?.destroy()}catch{}}},[v?.id]);
