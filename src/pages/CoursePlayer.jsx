import{useEffect,useRef,useState}from'react';import{useTeam}from'../context/TeamContext';import{useAuth}from'../context/AuthContext';import{supabase}from'../lib/supabase';import{totalVideoProgressXp,XP_PER_MINUTE,COMPLETION_XP}from'../lib/xp';
...
export default function CoursePlayer({courseId,videoId,back}){
 ...
 const playerHost=useRef(null),[watched,setWatched]=useState(progress[videoId]?.watched_seconds||0),[quizXp,setQuizXp]=useState(0),player=useRef(null),timer=useRef(null),last=useRef(null),watchedRef=useRef(progress[videoId]?.watched_seconds||0),xpWatchedRef=useRef(progress[videoId]?.watched_seconds||0);
 useEffect(()=>{const next=progress[videoId]?.watched_seconds||0;watchedRef.current=next;xpWatchedRef.current=next;setWatched(next);setTab('outline');setNotes([]);setTranscript(null);setBookmarks([]);setNote('');setAiContent(null);setAiLoading('');setAiError('')},[videoId]);
 useEffect(()=>{let alive=true;(async()=>{if(!user)return;const[{data:pr},{data:tests}]=await Promise.all([supabase.from('video_progress').select('watched_seconds,completed_at').eq('user_id',user.id),supabase.from('test_series').select('id').eq('team_id',team?.id)]);const chunks=await Promise.all((tests||[]).map(t=>supabase.rpc('get_test_attempt_history',{p_test_id:t.id})));const qxp=chunks.flatMap(x=>x.data||[]).filter(x=>x.user_id===user.id).reduce((n,x)=>n+Math.round(Number(x.percentage||0)/10),0);const learning=totalVideoProgressXp(pr||[]);if(alive)setGlobalXp(learning+qxp)})();return()=>{alive=false}},[user?.id,team?.id]);
 useEffect(()=>{if(!v)return;let mounted=true;const id=youtubeId(v.youtube_video_id||v.video_url||v.url);if(!id)return;ensureYouTubeApi().then(YT=>{if(!mounted||!playerHost.current)return;player.current=new YT.Player(playerHost.current,{videoId:id,width:'100%',height:'100%',playerVars:{rel:0,modestbranding:1,playsinline:1},events:{onReady:e=>{if(progress[v.id]?.last_position_seconds)e.target.seekTo(progress[v.id].last_position_seconds,true)},onStateChange:e=>{const Y=window.YT;if(e.data===Y.PlayerState.PLAYING){setPlaying(true);last.current=e.target.getCurrentTime();clearInterval(timer.current);timer.current=setInterval(()=>{const now=e.target.getCurrentTime(),delta=now-(last.current??now);if(delta>=0&&delta<=3.5){const before=watchedRef.current;watchedRef.current+=delta;setWatched(watchedRef.current);const beforeXp=Math.round(before/60*XP_PER_MINUTE),afterXp=Math.round(watchedRef.current/60*XP_PER_MINUTE);if(afterXp!==beforeXp)setGlobalXp(x=>x+(afterXp-beforeXp));save(now,e.target.getDuration())}last.current=now},2000)}else{setPlaying(false);clearInterval(timer.current);if(e.data===Y.PlayerState.ENDED)finish(e.target.getDuration())}}}});}).catch(()=>{});return()=>{mounted=false;clearInterval(timer.current);try{player.current?.destroy()}catch{}}},[v?.id]);
 ...
 async function loadAiContent(){if(!user||!v)return;const{data,error}=await supabase.from('video_ai_content').select('notes,summary,quiz').eq('playlist_video_id',v.id).eq('user_id',user.id).maybeSingle();if(!error)setAiContent(data||null)}
 async function generateAi(action){if(!user||!v)return;setAiLoading(action);setAiError('');const r=await supabase.functions.invoke('generate-video-ai',{body:{action,playlist_video_id:v.id}});if(r.error){setAiError(r.error.message||'AI generation failed.');setAiLoading('');return}setAiContent(r.data?.content||null);setAiLoading('')}
