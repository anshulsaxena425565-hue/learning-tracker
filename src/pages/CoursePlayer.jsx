...
export default function CoursePlayer({courseId,videoId,back}){
 const{user}=useAuth(),{courses,videos,progress,chapters}=useTeam();
 ...
 const[watched,setWatched]=useState(progress[videoId]?.watched_seconds||0),player=useRef(null),timer=useRef(null),last=useRef(null),watchedRef=useRef(progress[videoId]?.watched_seconds||0);
